...
Because overlap settings aren't set by afterglow, I've specified them using the -G option.
...
Burpdot now has 2 primary modes of operation, either output directly to a DOT file (with very limited formatting), or output as a "ref,url" CSV file. This is particular important for parsing these files with Afterglow. Mode is specified by the -m option. By default the option is "dot", but if you specify "csv" then a CSV is either printed to the screen or saved to a file (with the -o option). If the mode is set to CSV then the overlap option (-l) is ignored. You can download and read more about Afterglow here:
